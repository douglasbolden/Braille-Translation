...
exitCheck = 1
# receives an input from the user and then sends that input to the BrailleTranslation function.
while exitCheck == 1:
    print('Please enter a word to be converted to braille: (Anything other than the alphabetical letters:'
          ' a-z/A-Z WILL be ignored in output.)')
    i = input()
    # checks to make sure input isn't the word 'quit'
    # if the word is quit, it changes the exitCheck variable to 0 ending the while loop.
    if i == 'quit':
        exitCheck = 0
    else:
        BrailleTranslation(i)
